...
namespace Softnet.Tracker.ClientModel
{
    public class TCPController
    {
        ...
        public void Init(Site site, Client client, IChannel channel)
        {
            ...
            m_Client = client;
            m_Channel = channel;
            m_Channel.RegisterModule(Constants.Client.TcpController.ModuleId, OnMessageReceived);
        }
        ...
        void ProcessMessage_Request(byte[] message)
        {
            ...
            {
                service.TcpController.SendRequest(requestUid, virtualPort, m_Client, sessionTag);
            }
            else
            {
        ...
        SoftnetMessage EncodeMessage_RzvData(byte[] requestUid, byte[] connectionUid, int serverId, IPAddress serverIP)
        {
            ...
            return MsgBuilder.Create(Constants.Client.TcpController.ModuleId, Constants.Client.TcpController.RZV_DATA, asnEncoder);
        }
        ...
        SoftnetMessage EncodeMessage_RequestError(byte[] requestUid, int errorCode)
        {
            ...
            return MsgBuilder.Create(Constants.Client.TcpController.ModuleId, Constants.Client.TcpController.REQUEST_ERROR, asnEncoder);
        }
        ...
        SoftnetMessage EncodeMessage_ConnectionAccepted(byte[] requestUid)
        {
            ...
            var asnSequence = asnEncoder.Sequence;
            asnSequence.OctetString(requestUid);
            return MsgBuilder.Create(Constants.Client.TcpController.ModuleId, Constants.Client.TcpController.CONNECTION_ACCEPTED, asnEncoder);
        }
        ...
        SoftnetMessage EncodeMessage_AuthHash(byte[] requestUid, byte[] authHash, byte[] authKey2)
        {
            ...
            return MsgBuilder.Create(Constants.Client.TcpController.ModuleId, Constants.Client.TcpController.AUTH_HASH, asnEncoder);
        }
        ...
        SoftnetMessage EncodeMessage_AuthError(byte[] requestUid)
        {
            ...
            SequenceEncoder asnSequence = asnEncoder.Sequence;
            asnSequence.OctetString(requestUid);
            return MsgBuilder.Create(Constants.Client.TcpController.ModuleId, Constants.Client.TcpController.AUTH_ERROR, asnEncoder);
        }

        void OnMessageReceived(byte[] message)
        {
            byte messageTag = message[1];
            if (messageTag == Constants.Client.TcpController.REQUEST)
            {
                ProcessMessage_Request(message);
            }
            else if (messageTag == Constants.Client.TcpController.AUTH_KEY)
            {
