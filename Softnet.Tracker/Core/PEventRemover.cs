...
namespace Softnet.Tracker.Core
{
    class PEventRemover
    {
        static PEventRemover()
        {
            mutex = new object();
            s_eventList = new List<PEventInstance>();
            s_inProgress = false;
        }

        static object mutex; 
        static List<PEventInstance> s_eventList;
        static bool s_inProgress;

        public static void Add(PEventInstance eventInstance)
        {
            lock (mutex)
            {
                if (s_inProgress)
        ...
        {
            PEventInstance eventInstance = (PEventInstance)state;
            try
            {                
                SoftnetRegistry.EventController_DeletePEventInstance(eventInstance.InstanceId);
